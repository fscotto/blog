...
@UnitTest
class BlogResourceTest extends AbstractVerticleTestSuite {
  ...
  void shouldBe(VertxTestContext context) {
    mockingDependencyMethodCall(getArticles());
    getWebClient().get(NumberUtils.toInt(port), host, "/api/articles")
      .as(BodyCodec.string())
      .send(context.succeeding(response -> {
        assertThat(response.getDelegate().statusCode()).isEqualTo(200);
        assertThat(response.body()).isEqualTo(Json.encodePrettily(getArticles()));
        context.completeNow();
      }));
  }
